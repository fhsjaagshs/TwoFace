...
- (id)parseJsonResponse:(NSData *)data error:(NSError **)error {
    ...
    id result = [NSJSONSerialization JSONObjectWithData:[responseString dataUsingEncoding:NSUTF8StringEncoding allowLossyConversion:NO] options:NSJSONReadingMutableContainers error:nil];
...
- (void)connect {
    ...
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:url] cachePolicy:NSURLRequestReloadIgnoringLocalCacheData timeoutInterval:kTimeoutInterval];
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response {
    ...
    if ([_delegate respondsToSelector:@selector(request:didReceiveResponse:)]) {
        [_delegate request:self didReceiveResponse:(NSHTTPURLResponse*)response];
    }
}
