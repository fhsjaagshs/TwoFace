...
- (NSData *)synchronousURLRequest:(NSURL *)url;

// MGTwitterEngine implements this. include it here just so that we can use this private method
...
- (void)reAuthorizeFromCachedOAuthData {
    ...
    if (!_accessToken.key && !_accessToken.secret) {
        ...
        if ([_delegate respondsToSelector: @selector(cachedTwitterOAuthDataForUsername:)]) {
            accessTokenString = [(id)_delegate cachedTwitterOAuthDataForUsername:self.username];
...
- (void)requestRequestTokenSync {
    _requestToken = nil;
   // [self synchronousURLRequest:self.requestTokenURL];
    //NSData *data = [self synchronousURLRequest:self.requestTokenURL];
   ...
   // _requestToken = [[OAToken alloc] initWithHTTPResponseBody:responseBody];
    ...
    if (error) {
        [self outhTicketFailed:nil data:responseData];
    }*/
...
/*- (void)synchronousURLRequest:(NSURL *)url {
                        ...
                        responseType:(MGTwitterResponseType)responseType
{
    ...
    MGTwitterHTTPURLConnection *connection = [[MGTwitterHTTPURLConnection alloc] initWithRequest:theRequest 
                                                            delegate:self 
